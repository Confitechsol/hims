                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="javascript:void(0);"
                                                            class="fs-18 p-1 btn btn-icon btn-sm btn-soft-success rounded-pill editChargeCategoryBtn"
                                                            data-bs-toggle="modal" data-bs-target="#editChargeCategoryModal"
                                                            data-id="{{ $chargesCatogerys->id }}"
                                                            data-charge-type="{{ $chargesCatogerys->charge_type_id }}"
                                                            data-name="{{ $chargesCatogerys->name }}"
                                                            data-description="{{ $chargesCatogerys->description }}">
                                                            <i class="ti ti-pencil"></i>
                                                        </a>
                                                        <button type="button"
                                                        class="fs-18 p-1 btn btn-icon btn-sm btn-soft-danger rounded-pill deleteChargeCategoryBtn"
                                                        data-id="{{ $chargesCatogerys->id }}">
...
{{-- Pagination Links --}}
    ...
    <!-- Modal -->
    <div class="modal fade" id="editChargeCategoryModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                        ...
                        <div class="row gy-3">
                            <div class="col-md-12">
                                <label for="" class="form-label">Charge Type <span
                                        class="text-danger">*</span></label>
                                <select name="charge_type" id="edit_charge_type" class="form-select" required>
                                    <option value="">Select
                                    </option>
                                    @foreach ($charge_types as $charge_type)
                                        <option value="{{ $charge_type->id }}">
    ...
    <!-- Modal End -->
    <form id="deleteChargeCategoryForm"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            <td>
                ...
                data-bs-toggle="modal" data-bs-target="#editChargeCategoryModal"
                data-id="${item?.id}"
                data-charge-type="${item?.charge_type_id}"
                data-name="${item?.name}"
        ...
        `;
            ...
            document.addEventListener('click', function(e) {
                const button = e.target.closest('.editChargeCategoryBtn');
                if (!button) return;

                document.getElementById('edit_id').value = button.dataset.id;
                document.getElementById('edit_name').value = button.dataset.name;
                document.getElementById('edit_description').value = button.dataset.description;
                document.getElementById('edit_charge_type').value = button.dataset.chargeType;
            });
        });
document.addEventListener('click', function (e) {
    const button = e.target.closest('.deleteChargeCategoryBtn');
    if (!button) return;

    if (!confirm('Are you sure you want to delete this item?')) return;

    document.getElementById('delete_id').value = button.dataset.id;
    document.getElementById('deleteChargeCategoryForm').submit();
});
    </script>
@endsection
