            ...
            <div class="table-responsive">
                <table class="table table-bordered datatable">
                    ...
                    <div class="mt-3" id="pagination-wrapper">
                        @php
                            $currentPage = $dosages->currentPage();
                            $lastPage = $dosages->lastPage();
                        @endphp
                    
                        {{-- Previous --}}
                        @if ($dosages->onFirstPage())
                            <button class="btn btn-outline-secondary btn-sm me-1" disabled>« Prev</button>
                        @else
                            <a href="{{ $dosages->previousPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}" class="btn btn-outline-secondary btn-sm me-1">
                                « Prev
                            </a>
                        @endif
                        ...
                        @for ($page = 1; $page <= $lastPage; $page++)
                            ...
                            @else
                                <a href="{{ $dosages->url($page) }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}" class="btn btn-outline-secondary btn-sm me-1">
                                    {{ $page }}
                                </a>
                            @endif
                        @endfor
                        ...
                        {{-- Next --}}
                        @if ($dosages->hasMorePages())
                            <a href="{{ $dosages->nextPageUrl() }}{{ request('perPage') ? '&perPage=' . request('perPage') : '' }}" class="btn btn-outline-secondary btn-sm">
                                Next »
                            </a>
                        @else
