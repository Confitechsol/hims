    ...
    <div class="col-md-11">
        <div class="card shadow-sm border-0 mt-4">
            ...
            <div class="card-body">
                ...
                <form action="<?php echo e(route('radiology.billing.update', $bill->id)); ?>" method="POST" id="radiologyBillForm">
                    <?php echo csrf_field(); ?>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="testTable">
                                    ...
                                    <tbody id="testTableBody">
                                        ...
                                        <tr class="test-row">
                                            <td>
                                                <select name="tests[<?php echo e($index); ?>][radiology_id]" class="form-select test_name" required>
                                                    ...
                                                    <?php if(isset($tests) && count($tests) > 0): ?>
                                                        <?php $__currentLoopData = $tests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $test): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                            <option value="<?php echo e($test->id); ?>" 
                                                                ...
                                                                data-amount="<?php echo e($test->charge ? ($test->charge->standard_charge + ($test->charge->standard_charge * ($test->charge->taxCategory ? $test->charge->taxCategory->percentage : 0) / 100)) : 0); ?>"
                                                                <?php echo e($report->radiology_id == $test->id ? 'selected' : ''); ?>>
                                                                <?php echo e($test->test_name); ?> - ₹<?php echo e(number_format($test->charge ? ($test->charge->standard_charge + ($test->charge->standard_charge * ($test->charge->taxCategory ? $test->charge->taxCategory->percentage : 0) / 100)) : 0, 2)); ?>
                                            ...
                                            <td>
                                                <input type="number" name="tests[<?php echo e($index); ?>][report_days]" class="form-control report_days" value="<?php echo e($report->radiology->report_days ?? 0); ?>" min="0" readonly>
                                            </td>
                                            <td>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <a href="<?php echo e(route('radiology.billing.index')); ?>" class="btn btn-secondary">Cancel</a>
    ...
    $(document).on('change', '.test_name', function(e) {
        ...
        if (testId) {
            ...
            if (activateTpa && organisationId) {
                const url = `<?php echo e(url('/radiology/billing/api/tpa-charge')); ?>?test_id=${testId}&organisation_id=${organisationId}`;
                fetch(url)
...
function loadPatientPrescriptions(patientId) {
    const url = `<?php echo e(url('/radiology/billing/api/patient-prescriptions')); ?>/${patientId}`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
...
function loadPatientTpas(patientId) {
    ...
    const url = `<?php echo e(url('/radiology/billing/api/patient-tpas')); ?>/${patientId}`;
    const tpaDropdown = document.getElementById('tpa_dropdown');
...
function loadPrescriptionTests(prescriptionId) {
    const url = `<?php echo e(url('/radiology/billing/api/prescription-tests')); ?>/${prescriptionId}`;
    fetch(url)
        .then(response => response.json())
        .then(data => {
...
<?php echo $__env->make('layouts.adminLayout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp-8.2\htdocs\hims\resources\views/admin/radiology/billing/edit.blade.php ENDPATH**/ ?>